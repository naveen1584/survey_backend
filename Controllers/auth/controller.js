...
class authController extends BaseController {
            ...
            if (Object.keys(missing).length != 0) {
                ...
                if (checkRes.length > 0) {
                    ...
                    INSERT INTO users( userProfileName, DOB, userEmail,
                        userPhone, userPassword, roleID, addedBy, lastUpdatedBy)
                        VALUES
                        ('${userProfileName}','${DOB}','${userEmail}',
                        '${userPhone}','${hashPassword}','${roleID}',1,1)`;
            ...
            if (Object.keys(missing).length != 0) {
                ...
                if (result.length > 0) {
                    ...
                    if (!match) {
                        ...
                        const insertEmptyToken = `UPDATE users SET refreshToken = '' WHERE userEmail = '${userEmail}'`;
                        const resultEmptyToken = await DBSequelize.query(insertEmptyToken, {
                            type: Sequelize.QueryTypes.UPDATE
                        });
                        var token = createJWT(result[0].userEmail);
    ...
    async getUsersResetRequest(req, res) {
        try {
            const getQry = `SELECT * FROM users WHERE recoveryKey != ''`;
            const result = await DBSequelize.query(getQry, {
                type: Sequelize.QueryTypes.SELECT
            });
            return Response(res)({
    ...
    async deleteUser(req, res) {
        ...
        try {
            ...
            const deleteQry = `DELETE FROM users WHERE userId = '${userId}'`;
            const result = await DBSequelize.query(deleteQry, { type: Sequelize.QueryTypes.DELETE });
            return Response(res)({
